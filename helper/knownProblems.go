...
2) See what domain name was attempting to be resolved.  Should be a 'connecting' message prior to DNS failure`}
	...
	var earlyDisconnect = KnownProblem{Name: "Early-Disconnect", Disruption: kgpConnection, Logs: []byte("KGP libevent connection error MAIN main loop exited, return code: 5"), NextSteps: `Confirm this only
happens one time, at the end of the log after the Stop signal, CTRL-C (SIGINT), is sent.  If this happens during any other time there is potential for a bad connection or a Maki that has problems maintain a connection to a client.
...
This could be the customer network having problems maintaining the TCP tunnel or problems with the Keep Alive signal.  Look for any DEAD or LIVE signals in Sumo.`}
	...
	var noTunnelConnection = KnownProblem{Name: "No-Initial-Tunnel", Disruption: kgpConnection, Logs: []byte("sent reply 000000000000"), NextSteps: `This may not be a problem.  It can happen a handful of times 
...
Confirm that you can curl -vv a maki and the subdomain.  You should also use ping to see if there is packet loss.  Networking/IT team from customers side will probably need to intervene.`}
	...
	var sockMakiErr = KnownProblem{Name: "Socket-Maki-Conn", Logs: []byte("MAIN failed to connect KGP (socket error: socket error: No connection could be made because the target machine actively refused it.) MAIN failed to connect KGP socket error"), NextSteps: `Sauce labs lives at 3 IP blocks at the time of this writing.  So if we're unable to connect to a Maki or any other endpoint try pinging that endpoint from the Sauce Connect Host machine aka the host.
